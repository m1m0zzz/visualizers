...
export function PreviewCard({ item, frameSize }: Props) {
  ...
  return (
    <Card key={item.title} className="px-3 pt-6 pb-3">
      <CardContent className="flex flex-col gap-5 p-0">
        <div className="w-[100%] min-w-[100px] max-w-[280px] h-[100px] mx-auto">
          <WavePreview functions={item.functions} pos={pos / 100} />
        </div>
        <div className="mt-4 w-[100%] min-w-[100px] max-w-[280px] mx-auto flex gap-2">
          <Slider
            className="shrink"
            defaultValue={[pos]}
            max={100}
